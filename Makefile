...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=ncp \
		  -X github.com/cosmos/cosmos-sdk/version.ServerName=ncpd \
		  -X github.com/cosmos/cosmos-sdk/version.ClientName=ncpcli \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/ncpd.exe ./cmd/ncpd
	go build -mod=readonly $(BUILD_FLAGS) -o build/ncpcli.exe ./cmd/ncpcli
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/ncpd ./cmd/ncpd
	go build -mod=readonly $(BUILD_FLAGS) -o build/ncpcli ./cmd/ncpcli
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/ncpd
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/ncpcli
...
start-remote-sims:
	...
	docker build -f simulations/Dockerfile -t ncp/ncp-sim:master .
	# push that image to the hub
	docker push ncp/ncp-sim:master
