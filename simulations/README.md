...
>e.g. `ncp/ncp-sim:version1`, `go test ./app`
...
- create and submit a new array job (based of the job definition) with
  - image `ncp/ncp-sim:<some-version>`
...
- any changes needed to the code or script necessitates a rebuild:
  - `docker build -f simulations/Dockerfile -t ncp/ncp-sim:<some-version> .`
  - `docker push ncp/ncp-sim:<some-version>`
