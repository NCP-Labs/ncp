...
func NewFixtures(t *testing.T) *Fixtures {
	tmpDir, err := ioutil.TempDir("", "ncp_integration_"+t.Name()+"_")
	...
	return &Fixtures{
		...
		BuildDir:      buildDir,
		RootDir:       tmpDir,
		GaiadBinary:   filepath.Join(buildDir, "ncpd"),
		GaiacliBinary: filepath.Join(buildDir, "ncpcli"),
		GaiadHome:     filepath.Join(tmpDir, ".ncpd"),
		GaiacliHome:   filepath.Join(tmpDir, ".ncpcli"),
		RPCAddr:       servAddr,
		P2PAddr:       p2pAddr,
		Port:          port,
	}
